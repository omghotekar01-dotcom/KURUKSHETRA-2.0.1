...
REQUIRED_PRODUCTION_MARKERS = (
    'TRUSTKERNEL_ENV: production', 'TRUSTKERNEL_REQUIRE_API_KEY: "1"',
    'TRUSTKERNEL_ALLOW_LEGACY_ACTOR_HEADER: "0"', 'TRUSTKERNEL_REQUIRE_POLICY_APPROVAL: "1"',
    'TRUSTKERNEL_POLICY_FOUR_EYES: "1"', 'TRUSTKERNEL_OIDC_REQUIRE_HTTPS: "1"',
    'TRUSTKERNEL_DB_BACKEND: postgres', 'TRUSTKERNEL_QUOTA_BACKEND: redis',
    'TRUSTKERNEL_REDIS_STARTUP_PING: "1"', 'read_only: true', '- ALL', '- no-new-privileges:true',
)
REQUIRED_SECRET_GUARDS = (
    ...
    "TRUSTKERNEL_REDIS_URL:?set TRUSTKERNEL_REDIS_URL to a rediss:// endpoint",
)
REQUIRED_ATTESTATION_MARKERS = (
    "id-token: write", "attestations: write",
    "python backend/submission_manifest.py > trustkernel-submission-manifest.json",
    "uses: actions/attest@v4", "subject-path: trustkernel-submission-manifest.json",
    "sbom-path: backend/trustkernel-sbom.cdx.json",
    "github.event_name == 'push' && github.ref == 'refs/heads/main'",
)
REQUIRED_POSTGRES_MIGRATION_MARKERS = (
    'MIGRATION_LOCK_NAMESPACE = "trustkernel.schema.migrations"', "TRUSTKERNEL_MIGRATION_LOCK_TIMEOUT_MS",
    "pg_advisory_xact_lock(?)", "schema_migrations", "database migration checksum drift",
)
REQUIRED_POSTGRES_CI_MARKERS = (
    "image: postgres:16", "pg_isready -U trustkernel -d trustkernel", "TRUSTKERNEL_POSTGRES_TEST_URL",
    "pytest -q tests/test_v1416_postgres_integration.py",
)
REQUIRED_REDIS_QUOTA_MARKERS = (
    "class RedisQuotaBackend", "redis.call('TIME')", "redis.call('ZREMRANGEBYSCORE'",
    "redis.call('ZCOUNT'", "redis.call('ZADD'", "TRUSTKERNEL_QUOTA_BACKEND",
    "TRUSTKERNEL_REDIS_URL", "Redis server time is authoritative",
)
REQUIRED_REDIS_CI_MARKERS = (
...
def run() -> dict:
    ...
    checks.append({"name": "required_submission_assets", "passed": not missing, "missing": missing})
    ...
    checks.append({"name": "production_compose_posture", "passed": not missing_markers and not missing_secret_guards, "missing_markers": missing_markers, "missing_secret_guards": missing_secret_guards})
    ...
    checks.append({"name": "postgres_migration_coordination", "passed": not missing_migration_markers and not missing_postgres_ci_markers, "missing_migration_markers": missing_migration_markers, "missing_ci_markers": missing_postgres_ci_markers})

    missing_redis_markers = [marker for marker in REQUIRED_REDIS_QUOTA_MARKERS if marker not in quota_service]
    missing_redis_ci_markers = [marker for marker in REQUIRED_REDIS_CI_MARKERS if marker not in ci_workflow]
    bootstrap_quota_configured = "configure_quota_backend()" in bootstrap
    checks.append({
        "name": "redis_distributed_quota",
        "passed": not missing_redis_markers and not missing_redis_ci_markers and bootstrap_quota_configured,
        "missing_backend_markers": missing_redis_markers,
        "missing_ci_markers": missing_redis_ci_markers,
        "bootstrap_configured": bootstrap_quota_configured,
    ...
    checks.append({"name": "artifact_attestation_workflow", "passed": not missing_attestation_markers, "missing_markers": missing_attestation_markers})

    manifest = submission_manifest.build_manifest()
    checks.append({"name": "submission_manifest", "passed": manifest["complete"] and manifest["version"] == version, "asset_count": len(manifest["assets"]), "missing": manifest["missing"], "algorithm": manifest["algorithm"]})
